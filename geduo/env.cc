/*
 * @Author: Choubin
 * @Date: 2020-06-25 21:31:23
 * @LastEditors: Choubin
 * @LastEditTime: 2020-06-25 21:33:25
 * @FilePath: /geduo/geduo/env.cc
 * @Description:  
 */
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] == '-') {
            if (strlen(argv[i]) > 1) {
                ...
                GEDUO_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                                          << " val=" << argv[i];
                return false;
            }
        } else {
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
    ...
    static geduo::ConfigVar<std::string>::ptr g_server_work_path = geduo::Config::Lookup<std::string>("server.work_path");
